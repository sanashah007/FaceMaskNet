    ...
    def startLoop(self):
        ...
        while self.runThread:
            ...
            for _, nnet_packet in enumerate(self.nnet_packets):
                ...
                detectedObjects = nnet_packet.getDetectedObjects()

                if any(detectedObject.label == 1 for detectedObject in detectedObjects):
                    GPIO.output(18, True)
                else:
                    GPIO.output(18, False)

                if any(detectedObject.label == 2 for detectedObject in detectedObjects):
                    GPIO.output(17, True)
                else:
