...
class DepthAI:
    ...
    def startLoop(self):
        ...
        with open('MaskLog.csv', mode='w') as masklog_file:
            maskLog_writer = csv.writer(masklog_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            maskLog_writer.writerow(['People_Detected', 'Mask', 'No_Mask'])
        ...
        logging.basicConfig(filename='sample.csv', level=logging.DEBUG, format='%(asctime)s %(message)s')
        logging.info(" Ms, People detected, Mask , NoMask")
        ...
        while self.runThread:
            ...
            for _, nnet_packet in enumerate(self.nnet_packets):
                ...
                detected_objects = nnet_packet.getDetectedObjects()
                if len(detected_objects) > 0:
                #     with open('MaskLog.csv', mode='w') as masklog_file:
                #         maskLog_writer = csv.writer(masklog_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
                #         maskLog_writer.writerow([len(detected_objects), sum(detectionsperframe1.label == 2 for detectionsperframe1 in detected_objects), sum(detectionsperframe1.label == 1 for detectionsperframe1 in detected_objects)])

                    # logging.basicConfig(filename='sample.log', level=logging.DEBUG, format='%(asctime)s %(message)s')
                    # logging.info('\n')
                    # logging.info(("length of detected object with label 1 " + str(sum(detectionsperframe1.label == 1 for detectionsperframe1 in detected_objects))))
                    # logging.info(("Length of detected object " + str(len(detected_objects))))
                    # logging.info(("length of detected object with label 2 " + str(sum(detectionsperframe1.label == 2 for detectionsperframe1 in detected_objects))))
                  
                    logging.info((' ,' + str(len(detected_objects))+' ,' + str(sum(detectionsperframe1.label == 1 for detectionsperframe1 in detected_objects)) + ' ,' + str(sum(detectionsperframe1.label == 2 for detectionsperframe1 in detected_objects))))
                    
                    if any(detectionsperframe.label == 1 for detectionsperframe in detected_objects):
                            GPIO.output(18,True)
                    else:         
                            GPIO.output(18,False)

                    if any(detectionsperframe.label == 2 for detectionsperframe in detected_objects):
                            GPIO.output(21, True)
                    else: 
                            GPIO.output(21, False)
                #There are no persons detected, so turn LED OFF
                else:
                    GPIO.output(18,False)
                    GPIO.output(21, False)
